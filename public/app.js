import { DEMO_WEEKS, SAMPLE_MESSAGE, compareDemoWeeks, computeDemoRisks, extractDemoUpdate, getDemoItems, scanDemoPrivacy } from "./demo-intelligence.js?v=20260722-evidence-v2";
  ...
  return html`<div class="demo-two-column">
    <form class="demo-panel capture-form" id="demo-capture-form">
      ...
      <div class="demo-form-grid">
        <label>Source title<input name="title" required value="Fictional H1 Review & H2 Plan | Portfolio MMR"></label>
        <label>Source date<input name="date" type="date" required value="2026-07-20"></label>
...
function captureTabData() {
  const extraction = state.demo.extraction || { records: [], risks: [], decisions: [], actions: [], gates: [], pmmRequests: [], questions: [] };
  ...
  if (state.demo.reviewTab === "gates") return extraction.gates;
  if (state.demo.reviewTab === "questions") return extraction.questions;
  return extraction.records;
}

function renderCaptureReview() {
  const extraction = state.demo.extraction || { records: [], risks: [], decisions: [], actions: [], gates: [], pmmRequests: [], questions: [], metrics: {} };
  const tabs = [["updates", extraction.records.length], ["actions", extraction.actions.length], ["gates", extraction.gates.length], ["risks", extraction.risks.length], ["decisions", extraction.decisions.length], ["questions", extraction.questions.length]];
...
function renderCaptureComplete() {
  const bundle = state.demo.importBundle || { records: [], risks: [], decisions: [], actions: [], gates: [], questions: [], metrics: {} };
  ...
  return html`<section class="demo-panel import-summary-panel">
    ...
    <div class="import-stats"><div><strong>${bundle.actions.length}</strong><span>Actions captured</span></div><div><strong>${bundle.gates?.length || 0}</strong><span>Approval gates</span></div><div><strong>${bundle.risks.length}</strong><span>Attention signals</span></div><div><strong>${bundle.questions?.length || 0}</strong><span>Confirmation questions</span></div></div>
    ...
    <div class="import-preview">${bundle.records.map((record) => `<article><div><strong>${escapeHtml(record.product)}</strong><span class="status-pill ${statusClass(record.status)}">${escapeHtml(record.status)}</span></div><p>${escapeHtml(record.sourceExcerpt || "Approved structured topic")}</p><small>${record.progress == null ? "Progress not reported — no value inferred" : `${record.progress}% · source reported`} · ${escapeHtml(record.owner)}</small></article>`).join("")}</div>
...
function bindSmartCaptureEvents() {
  ...
  document.querySelectorAll("[data-approve-draft]").forEach((input) => input.addEventListener("change", () => {
    const collections = ["records", "actions", "gates", "risks", "decisions", "questions"];
    const record = collections.flatMap((key) => state.demo.extraction[key] || []).find((entry) => entry.id === input.dataset.approveDraft);
    if (record) record.approved = input.checked;
    render();
  }));
  ...
  document.querySelector("#approve-demo-records")?.addEventListener("click", () => {
    const approved = (key) => (state.demo.extraction[key] || []).filter((record) => record.approved);
    const bundle = {
      importedAt: new Date().toISOString(),
      summary: state.demo.extraction.summary,
      metrics: state.demo.extraction.metrics,
      records: approved("records"),
      risks: approved("risks"),
      decisions: approved("decisions"),
      actions: approved("actions"),
      gates: approved("gates"),
...
function filteredDemoItems(items) {
  ...
  if (filter === "missing-progress") return items.filter((item) => !Number.isFinite(item.progress));
...
function getFictionalIntelligenceItems() {
  ...
  return items.map((item) => {
    ...
    const capturedActions = (bundle.actions || []).filter((entry) => entry.product === item.name);
    const capturedGate = (bundle.gates || []).find((entry) => entry.product === item.name);
    const capturedDecisions = (bundle.decisions || []).filter((entry) => entry.product === item.name);
    const capturedRisks = (bundle.risks || []).filter((entry) => entry.product === item.name);
    const capturedQuestions = (bundle.questions || []).filter((entry) => entry.product === item.name);
    const isClosed = capturedDecisions.some((entry) => entry.recordType === "closed_finding");
    const isFollowUp = capturedRisks.some((entry) => entry.recordType === "follow_up_risk");
    const isPriority = capturedDecisions.some((entry) => entry.recordType === "priority");
    const evidenceCount = capturedActions.length + capturedDecisions.length + capturedRisks.length + capturedQuestions.length + (capturedGate ? 1 : 0);
    ...
    return addExecutionMetrics({
      ...
      metric: hasProgress ? "Source-reported progress" : "Progress not reported",
      actual: hasProgress ? imported.progress : null,
      target: hasProgress ? 100 : null,
      unit: hasProgress ? "%" : "",
      progress: hasProgress ? imported.progress : null,
      change: hasProgress && Number.isFinite(item.previous) ? imported.progress - item.previous : null,
...
function computeCapturedRisks(items) {
  const bundle = state.demo.importBundle;
  if (!bundle) return [];
  const visibleIds = new Map(items.map((item) => [item.name, item.id]));
  return [...(bundle.gates || []), ...(bundle.risks || []), ...(bundle.questions || [])]
    .filter((record) => visibleIds.has(record.product))
    .map((record) => ({
      itemId: visibleIds.get(record.product),
      itemName: record.product,
      owner: record.owner || "Unassigned",
      rule: record.recordType === "gate" ? "Approval gate" : record.requiresConfirmation ? "Needs confirmation" : "Captured attention signal",
      severity: record.recordType === "gate" ? "critical" : record.severity || "warning",
      detail: record.text,
    }));
}

function computeEvidenceRisks(items) {
  const bundle = state.demo.importBundle || {};
  const visibleIds = new Map(items.map((item) => [item.name, item.id]));
  const managementSignals = [
    ...(bundle.gates || []),
    ...(bundle.risks || []).filter((record) => record.recordType === "follow_up_risk"),
    ...(bundle.questions || []),
  ];
  return managementSignals.filter((record) => visibleIds.has(record.product)).map((record) => ({
    itemId: visibleIds.get(record.product), itemName: record.product, owner: record.owner || "Unassigned",
    rule: record.recordType === "gate" ? "Approval gate" : record.recordType === "follow_up_risk" ? "Time-sensitive follow-up" : "Needs confirmation",
    severity: record.recordType === "gate" ? "critical" : "warning", detail: record.text,
  }));
}
...
function renderExecutionIntelligence() {
  ...
  const content = html`<section class="control-room">
    ...
    <section class="demo-panel system-map-panel"><div class="demo-panel-heading"><div><span class="demo-kicker">System view</span><h2>Interactive execution map</h2><p>Use the stage board to locate delay, then use this map to see portfolio distribution, dependencies and evidence freshness as one connected system.</p></div><span class="demo-badge neutral">Click any node</span></div>${renderExecutionMap(items)}</section>
...
function renderExecutionMap(items) {
  ...
  const evidenceMode = items.some((item) => item.newFromCapture);
  return `<div class="map-legend">${evidenceMode ? `<span><i class="priority"></i>Priority</span><span><i class="confirm"></i>Confirm</span><span><i class="gate"></i>Gate</span><span><i class="closed"></i>Closed</span><span>Size = captured evidence count</span>` : `<span><i class="on-track"></i>Active</span><span><i class="at-risk"></i>Attention</span><span><i class="blocked"></i>Gate</span><span>${live ? "Fixed size = progress unavailable" : "Size = reported progress"}</span><span>Ring = evidence freshness</span>`}</div><svg class="execution-map" viewBox="0 0 ${width} ${height}" role="img" aria-label="Execution map of ${live ? "live read-only" : "fictional"} workstreams"><defs><marker id="arrow" markerWidth="8" markerHeight="8" refX="7" refY="3" orient="auto"><path d="M0,0 L0,6 L8,3 z"/></marker></defs><g class="dependency-lines">${lines}</g>${items.map((item) => { const p = positions[item.id]; const radius = evidenceMode ? 33 + Math.min(12, (item.evidenceCount || 1) * 2) : Number.isFinite(item.progress) ? 28 + item.progress * .14 : 34; const shortName = item.name.length > 20 ? `${item.name.slice(0, 18)}…` : item.name; const centerValue = evidenceMode ? item.evidenceValue : Number.isFinite(item.progress) ? `${item.progress}%` : "—"; return `<g class="map-node ${evidenceMode ? `evidence-${item.evidenceType}` : statusClass(item.status)} ${state.demo.selectedId === item.id ? "selected" : ""}" tabindex="0" role="button" data-select-demo="${item.id}" transform="translate(${p.x} ${p.y})"><circle class="freshness-ring ${!evidenceMode && item.fresh >= (live ? 14 : 7) ? "stale" : ""}" r="${radius + 7}"/><circle class="node-body" r="${radius}"/><text class="node-progress" y="${evidenceMode ? -2 : 4}">${escapeHtml(centerValue)}</text>${evidenceMode ? `<text class="node-signal" y="17">${escapeHtml(item.evidenceLabel)}</text>` : ""}<text class="node-name" y="${radius + 22}">${escapeHtml(shortName)}</text></g>`; }).join("")}</svg>`;
}

function renderSignalHeatmap(items) {
  const columns = [...new Set(items.map((item) => item.area || "Unclassified"))].slice(0, 8);
  const evidenceMode = items.some((item) => item.newFromCapture);
  const rows = evidenceMode
    ? [{ key: "missing-progress", label: "Progress not reported", test: (i) => !Number.isFinite(i.progress) }, { key: "missing-date", label: "Missing due date", test: (i) => !i.due || i.due === "Not reported" }, { key: "approval-gate", label: "Approval gate", test: (i) => Boolean(i.capturedGate) }, { key: "needs-confirmation", label: "Needs confirmation", test: (i) => Boolean(i.requiresConfirmation) }]
    : [{ key: "stale", label: "Stale updates", test: (i) => i.fresh >= 7 }, { key: "blocked", label: "Blocked", test: (i) => i.status === "Blocked" }, { key: "repeat-risk", label: "Repeated risks", test: (i) => i.riskCount >= 3 }, { key: "low-completeness", label: "Low completeness", test: (i) => i.completeness < 75 }];
...
function renderExecutionDrawer(item, risks) {
  ...
  return html`<aside class="demo-drawer" aria-label="Selected workstream details"><div class="drawer-heading"><div><span class="demo-kicker">${item.isLive ? "Reported + Derived" : item.newFromCapture ? "Captured evidence" : "Act"}</span><h2>${escapeHtml(item.name)}</h2><span class="status-pill ${statusClass(item.status)}">${escapeHtml(item.status)}</span>${item.newFromCapture ? `<span class="demo-badge success">${escapeHtml(item.evidenceLabel)}</span>` : ""}</div><button class="secondary" id="close-demo-drawer" aria-label="Close details">×</button></div>${item.newFromCapture ? `<div class="captured-evidence-summary"><div><strong>${item.capturedActions?.length || 0}</strong><span>Actions</span></div><div><strong>${item.capturedDecisions?.length || 0}</strong><span>Decisions</span></div><div><strong>${item.capturedGate ? 1 : 0}</strong><span>Gates</span></div><div><strong>${item.capturedQuestions?.length || 0}</strong><span>Questions</span></div></div>` : ""}<div class="drawer-metric ${hasMetric ? "" : "unavailable"}"><span>${escapeHtml(item.metric)}</span><strong>${hasMetric ? `${item.actual}${escapeHtml(item.unit)}` : "Not reported"}</strong>${hasMetric ? `<div><i style="width:${item.progress}%"></i></div><small>${item.progress}% of ${item.target}${escapeHtml(item.unit)} target · reported, not forecast</small>` : `<small>No quantitative evidence was present. The system did not infer a value.</small>`}</div><div class="drawer-grid"><div><span>Owner · Reported</span><strong>${escapeHtml(item.owner)}</strong></div><div><span>Target date · Reported</span><strong>${escapeHtml(item.due)}</strong></div><div><span>Freshness · Derived</span><strong>${item.fresh >= 999 ? "Unavailable" : `${item.fresh} days`}</strong></div><div><span>Evidence records</span><strong>${item.evidenceCount || "Unavailable"}</strong></div></div><h3>Why this needs attention</h3><div class="drawer-risks">${risks.length ? risks.map((risk) => `<div><span class="risk-dot ${risk.severity}"></span><p><strong>${escapeHtml(risk.rule)}</strong><small>${escapeHtml(risk.detail)}</small></p></div>`).join("") : `<p class="muted">No active management attention signal.</p>`}</div>${item.sourceExcerpt ? `<h3>Sanitized source excerpt</h3><p class="source-excerpt">${escapeHtml(item.sourceExcerpt)}</p>` : ""}<h3>Dependencies</h3><p>${item.deps.length ? item.deps.map((dep) => escapeHtml(dep.replaceAll("-", " "))).join(" · ") : item.isLive ? "Unavailable — no structured dependency field in the source Board." : "No structured upstream dependency captured"}</p>${item.isLive ? `<div class="read-only-callout"><strong>Read-only evidence</strong><span>This panel cannot edit or write back to the shared Board.</span></div>` : `<details class="metric-builder"><summary>Configure metric</summary><label>Template<select><option>Percentage complete</option><option>Count toward target</option><option>Milestone weighted</option></select></label><label>Metric name<input value="${escapeHtml(item.metric)}"></label><div class="demo-form-grid"><label>Current<input type="number" value="${item.actual ?? ""}"></label><label>Target<input type="number" value="${item.target ?? ""}"></label></div><button type="button" id="save-demo-metric">Save locally</button></details><div class="drawer-actions"><button class="secondary" id="demo-add-evidence">Add fictional evidence</button><button id="demo-create-action">Create demo action</button></div><p class="drawer-footnote">Demo-only actions are stored locally and never call the production API.</p>`}</aside>`;
}
